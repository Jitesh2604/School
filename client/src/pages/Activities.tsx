...
const schedule = [
  { time: "8:00 – 8:30 AM", label: "Welcome Time", icon: "👋", desc: "Greetings, attendance & morning circle" },
  { time: "8:30 – 9:00 AM", label: "Circle Time", icon: "🔵", desc: "Group discussion, show & tell, calendar activity" },
  { time: "9:00 – 10:30 AM", label: "Activity Time", icon: "✏️", desc: "Art, craft, music, or learning games" },
  { time: "10:30 – 11:00 AM", label: "Lunch Break", icon: "🍱", desc: "Healthy lunch & good eating habits" },
  { time: "11:00 – 11:30 AM", label: "Nap Time", icon: "😴", desc: "Rest & quiet relaxation" },
  { time: "11:30 AM – 12:30 PM", label: "Play Time", icon: "⚽", desc: "Outdoor games & free play" },
];

const benefits = [
  { icon: "🎭", title: "Creativity", desc: "Open-ended activities nurture imagination and innovative thinking from an early age." },
  { icon: "🤝", title: "Social Skills", desc: "Group play teaches children to share, cooperate, and form meaningful friendships." },
  { icon: "💬", title: "Communication", desc: "Stories, songs, and discussions build vocabulary and confidence in self-expression." },
  { icon: "🏃", title: "Physical Development", desc: "Active play builds strength, coordination, and healthy habits that last a lifetime." },
];
    ...
    <div className="min-h-screen bg-[#FFFBF5] font-sans">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#FFF3E0] via-[#FFF8F0] to-[#FCE4EC] py-20 px-4 text-center">
        ...
        <div className="relative z-10 mx-auto max-w-2xl">
          ...
          <p className="mt-4 text-lg text-[#5D4037]">
            Fun-filled learning experiences for every child — where curiosity meets joy.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="mb-10 text-center">
          ...
          <p className="mt-2 text-[#6D6D6D]">Eight engaging activities crafted to bring out the best in every child.</p>
        </div>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {activities.map((act) => (
            ...
            >
              <div className={`mb-4 inline-flex h-14 w-14 items-center justify-center rounded-xl ${act.iconBg} text-3xl shadow-sm`}>
                {act.icon}
              </div>
              <h3 className="mb-2 text-lg font-bold text-[#2D2D2D]">{act.title}</h3>
              <p className="text-sm leading-relaxed text-[#6D6D6D]">{act.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-br from-[#E8F5E9] to-[#F1F8E9] py-16 px-4">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <p className="mt-2 text-[#4A6741]">A balanced, nurturing routine that children love and parents trust.</p>
          </div>
                ...
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div className="flex-1 rounded-2xl bg-white p-4 shadow-sm">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <h3 className="font-bold text-[#2D2D2D]">{item.label}</h3>
                    </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="mb-10 text-center">
          ...
          <p className="mt-2 text-[#6D6D6D]">
            Every game, song, and story is purposefully designed to build foundational skills.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {benefits.map((b) => (
            <div key={b.title} className="rounded-2xl bg-white p-6 text-center shadow-md ring-1 ring-gray-100 transition hover:shadow-lg">
              <div className="mb-3 text-4xl">{b.icon}</div>
              <h3 className="mb-2 text-lg font-bold text-[#2D2D2D]">{b.title}</h3>
              <p className="text-sm leading-relaxed text-[#6D6D6D]">{b.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#FFF8F0] py-16 px-4">
        <div className="mx-auto max-w-5xl">
          <div className="mb-10 text-center">
            ...
            <p className="mt-2 text-[#6D6D6D]">Moments that tell the story of happy, thriving children.</p>
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {galleryImages.map((img) => (
              ...
              >
                ...
                <span className="mt-3 text-sm font-semibold text-[#4A4A4A]">{img.label}</span>
              </div>
            ))}
          </div>
